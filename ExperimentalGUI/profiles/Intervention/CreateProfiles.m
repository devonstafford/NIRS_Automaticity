...
velNonDom = velDom;
for i = 1:5
    velDom = [velDom,1*ones(1,150),0.75*ones(1,50)];
    velNonDom = [velNonDom, 0.5*ones(1,150),0.75*ones(1,50)];
end
velDom = [velDom, 1*ones(1,150), 0.75*ones(1,600)];
velNonDom = [velNonDom, 1*ones(1,150), 0.75*ones(1,600)];

close all
figure;
area([600,1750],[1,1],0.5,'FaceColor','#edebe4','EdgeColor','#edebe4');

hold on;
area([0,150],[1,1],0.5,'FaceColor','#e4f7f6','EdgeColor','#e4f7f6');
area([1750,2200],[1,1],0.5,'FaceColor','#e4f7f6','EdgeColor','#e4f7f6');
...
breakloc = [300,450,800,1200,1600,2050];
for i = breakloc
    xline(i,'k--','LineWidth',2);
end
f= get(gca,'Children');
legend(f([1, 7, 8,9,end]),'Break (5mins)','Non-Dominant Leg','Dominant Leg','Overground','Switching');
