...
stimulusClean = Dictionary();
for stimIdx = eventKeyIdx
    idx = find(~cellfun(@isempty,regexp(sortedLabels,['^' raw.stimulus.keys{stimIdx} '[0-9]'])));
    eventDuration = allTimeSorted(idx+1) - allTimeSorted(idx);  
    stimulusClean(raw.stimulus.keys{stimIdx}) = nirs.design.StimulusEvents(raw.stimulus.keys{stimIdx},allTimeSorted(idx),eventDuration,[1;1;1;1;1;1]);
end
raw.stimulus = stimulusClean;
